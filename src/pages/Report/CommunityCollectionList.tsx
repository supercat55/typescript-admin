...
@connect(({ menu, loading, report, global }) => ({
  ...
  communityCollectionList: report.communityCollectionList,
  communityCollectionTotal: report.communityCollectionTotal,
...
class CommunityCollectionList extends PureComponent<IProps, IState> {
  ...
  private columns = [
    ...
    { title: '收缴率', dataIndex: 'collectionRate', key: 'collectionRate' },
  ]
  ...
  handleSearchList = () => {
    ...
    dispatch({
      type: 'report/getCommunityCollectionList',
      payload: params
    })
  }
  ...
  render() {
    ...
    const { communityCollectionList, communityCollectionTotal, tableLoading, globalPageSubMenu } = this.props;

    const pagination = {
      total: communityCollectionTotal,
      current: pageNum,
      pageSize,
    };

    return (
      <PageWrapper title='社区收缴率报表'>
        {
          globalPageSubMenu.COMMUNITY_COLLECTION_SEARCH_OR_EXPORT || globalPageSubMenu.COMMUNITY_COLLECTION_SEARCH_OR_EXPORT_MERCHANT ? 
          <PageSearchForm 
        ...
        <StandardTable
          ...
          dataSource={communityCollectionList}
