...
class BottomBannerTemplateActions extends PureComponent<IProps, IState> {
  state = {
    mode: 'create',
  ...
  init = async() => {
    ...
    if (mode === 'default') {
      const result = await dispatch({
        ...
        payload: { templateType: 1, bannerType: 2 }
      })
  ...
  render() {
    ...
    return (
      <PageWrapper 
        title={GetPageTitleByMode(mode, '底部banner模版')}
        showBack
        customBreadcrumbmap={[
          { name: '参数管理', url: '' },
          { name: '底部banner模版管理', url: '/parameter/bottom-banner' },
          { name: GetPageTitleByMode(mode, '底部banner模版') },
        ]}
      >
                      ...
                      >
                        {
                          ...
                          (<img src={item.url} style={{ width: '100%', height: 130 }}/>) : 
                          <div>
...
export default Form.create()(BottomBannerTemplateActions);
