...
import { AuthOperationActionsType } from '@/services/auth';
...
@connect(({ loading, authOperation, global }) => ({
  allOrganizationOptions: authOperation.allOrganizationOptions,
  roleListByAttribute: global.roleListByAttribute,
  pageLoading: loading.models['authOperation'],
}))
class AuthOperationActionsPage extends PureComponent<IProps, IState> {
  state = {
    mode: 'create',
  ...
  init = async() => {
    ...
    await dispatch({
      ...
      payload: { roleAttribute: 0 }
    })

    if (id && mode !== 'create') {
      let result = await dispatch({
        type: 'authOperation/getAuthOperationDetail',
        payload: { id }
      })
  ...
  @Bind()
  @Debounce(500)
  async handleValidateFormItem(rule, value, next) {
    const { mode } = this.state;
    const { dispatch, form: { setFields } } = this.props;

    if (value && mode === 'create') {
      let result = await dispatch({
        type: 'authOperation/validateAccount',
        payload: {
          accountNum: value
        }
      })
  ...
  handleSubmit = e => {
    ...
    form.validateFields(async(err, values) => {
      ...
      const { accountNum, name, organizeId, roleId, phoneNum, email } = values;

      const params: AuthOperationActionsType = {
        accountNum,
        name,
        organizeId,
        roleId
      ...
      if (id && mode === 'edit') {
        ...
        dispatch({
          type: 'authOperation/editAuthOperation',
          payload: params
        });
      ...
      let result = await dispatch({
        type: 'authOperation/createAuthOperation',
        payload: params
      })
  ...
  render() {
    const { mode } = this.state;
    const { allOrganizationOptions, roleListByAttribute, pageLoading, form: { getFieldDecorator } } = this.props;
          ...
          <Form {...formItemLayout} onSubmit={this.handleSubmit}>
            <FormItem label={'账号ID'} hasFeedback>
              {getFieldDecorator('accountNum',{
                ...
                rules: [
                  { required: true, message: '账号ID为空' },
                  { max: 20, message: '账号ID不能超过20个字' },
                  { validator: this.handleValidateFormItem }
                ]
              })(
            ...
            <FormItem label={'所属组织'}>
              {getFieldDecorator('organizeId', {
                ...
                <Select placeholder="请选择所属组织" disabled={mode === 'detail'}>
                  {allOrganizationOptions.map(item => (
                    <Option value={item.id} key={item.id}>
                      <span>{`${item.name}-${ORGANIZ_PROPERTY_TYPES_DESC[item.attribute]}`}</span>
                    </Option>
                  ))}
                </Select>
              )}
...
export default Form.create()(AuthOperationActionsPage);
