...
import { Form, Input, Select, Spin, Tree, Icon, Button, Modal, Row, Col } from 'antd';
...
import { ROLE_PROPERTY_TYPES } from '@/utils/const';
...
@connect(({ menu, loading, authRole, global }) => ({
  globalPageSubMenu: menu.globalPageSubMenu,
  allOrganizationOptions: authRole.allOrganizationOptions,
  moduleListByRoleType: global.moduleListByRoleType,
  pageLoading: loading.models['authRole'],
}))
class AuthRoleActionsPage extends PureComponent<IProps, IState> {
  state = {
    mode: 'create',
  ...
  init = async() => {
    ...
    if (id && mode !== 'create') {
      let result = await dispatch({
        type: 'authRole/getAuthRoleDetail',
        payload: { id }
      })
  ...
  handleGetModuleList = async() => {
    ...
    const roleType = form.getFieldValue('roleAttribute');

    let moduleList = await dispatch({
      type: 'global/getRoleModuleListByRoleType',
      payload: {
        type: roleType === 0 ? 'operation' : 'merchant'
      }
    })
  ...
  async handleValidateRoleName(rule, value, next) {
    ...
    if (value && mode === 'create') {
      let result = await dispatch({
        type: 'authRole/validateRoleName',
        payload: {
          roleName: value
        }
      })
  ...
  handleSubmit = e => {
    ...
    form.validateFields((err, values) => {
      ...
      const { roleName, roleAttribute } = values;
      ...
      >
        <Spin spinning={pageLoading !== undefined ? pageLoading : false}>
          <Form {...formItemLayout} onSubmit={this.handleSubmit}>
            ...
            <FormItem label={'角色属性'}>
              {getFieldDecorator('roleAttribute', {
                  initialValue: 0,
                  rules: [
                    { required: true, message: '请选择角色属性' }
                  ]
              })(
                <Select disabled={mode !== 'create'} placeholder="请选择角色属性" onChange={this.handleRolePropertyTypeChange}>
                  {ROLE_PROPERTY_TYPES.map(item => (
...
export default Form.create()(AuthRoleActionsPage);
