...
@connect(({ menu, authMerchantOperator, loading }) => ({
  globalPageSubMenu: menu.globalPageSubMenu,
  merchantList: authMerchantOperator.merchantList,
  merchantTotal: authMerchantOperator.merchantTotal,
  tableLoading: loading.effects['authMerchantOperator/getMerchantOperatorList'],
}))
class AuthMerchantPage extends PureComponent<IProps, IState> {
        ...
        if (Number(record.status) === 0) {
          return (
            <div className='table-actions'>
              {globalPageSubMenu.CHECK_MERCHANT_OPERATOR_DETAIL && <span onClick={() => this.handleActions('detail', record)}>查看</span>}
            </div>
          )
        } else {
          return (
            <div className='table-actions'>
              {globalPageSubMenu.RESET_MERCHANT_OPERATOR_PWD && <span onClick={() => this.handleActions('reset', record)}>重置密码</span>}
              {globalPageSubMenu.MERCHANT_OPERATOR_CREATE_OR_EDIT && <span onClick={() => this.handleActions('edit', record)}>编辑</span>}
              {
                globalPageSubMenu.TOOGLE_MERCHANT_OPERATOR_STATUS &&
                <Popconfirm
                  title="是否确认停用此账号？"
                  okText="确定"
                  cancelText="取消"
  ...
  handleSearchList = () => {
    ...
    const { pageNum, pageSize, searchFormValues: { merchantName, status, condition } } = this.state;
    ...
    dispatch({
      type: 'authMerchantOperator/getMerchantOperatorList',
      payload: params
    })
  }
  ...
  handleActions = (mode, info) => {
    switch (mode) {
      case 'create':
        router.push({
          pathname: '/auth/merchant-operator/actions',
      ...
      case 'edit':
        router.push({
          pathname: `/auth/merchant-operator/actions/${info.id}`,
          search: stringify({ mode })
        })
        break;
      case 'reset':
  ...
  handleResetConfirm = () => {
    ...
    form.validateFields(async(err, values) => {
      ...
      let result = await dispatch({
        type: 'authMerchantOperator/resetAuthMerchantPwd',
        payload: {
          initialPassword,
  ...
  handleDisableItem = async({ id }) => {
    ...
    let result = await dispatch({
      type: 'authMerchantOperator/disableAuthMerchantOperator',
      payload: { 
        id,
        status: 0
      }
  ...
  render() {
    ...
    return (
      <PageWrapper title='商户管理员账号'>
        {
          globalPageSubMenu.MERCHANT_OPERATOR_SEARCH &&
          <PageSearchForm 
        ...
        { 
          globalPageSubMenu.MERCHANT_OPERATOR_CREATE_OR_EDIT &&
          <Button 
            type='primary'
            icon='plus'
